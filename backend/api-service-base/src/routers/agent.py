...
async def get_menu_for_agent(
    ...
    """
    logger.info("Fetching menu for voice agent")
    ...
    unavailable = db.query(Product).filter(Product.is_available == False).all()
    ...
    logger.info(f"Menu loaded: {len(all_products)} products, {len(menu_data['unavailable'])} unavailable")
...
async def get_recommendations(
    ...
    """
    logger.info(f"Getting recommendations: classification={classification}, max_spice={max_spice}")
...
async def search_by_preferences(
    ...
    """
    logger.info(f"Search by preferences: query={query}, sin_picante={sin_picante}")

    products_query = db.query(Product).filter(Product.is_available == True)
...
async def get_upsell_suggestion(
    ...
    """
    logger.info(f"Getting upsell suggestions for product {product_id}")
    ...
    # Buscar bebidas si el producto es comida
    ...
    # Buscar producto estrella de la misma categoría
