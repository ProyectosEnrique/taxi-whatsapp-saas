      ...
      <div class="modal-container">
        ...
        <div class="conversation-area" ref="conversationRef">
          ...
          <div v-if="chatStore.messages.length === 0" class="welcome-message">
            ...
            <p>Tu asistente personal de Taxi Rápido</p>
            <p class="text-sm text-gray-600 mt-2">Puedes escribir o usar tu voz para solicitar un viaje</p>

            <!-- Quick Actions -->
            <div class="quick-actions">
              <button class="quick-action-btn" @click="handleQuickMessage('Necesito un taxi ahora')">
                <span class="action-icon">🚕</span>
                <span class="action-label">Solicitar taxi</span>
              </button>
              <button class="quick-action-btn" @click="handleQuickMessage('¿Cuánto cuesta a la estación?')">
                <span class="action-icon">💰</span>
                <span class="action-label">Ver tarifas</span>
              </button>
...
// Tenant ID para Taxi
const TENANT_ID = 'tenant_taxi_001'

onMounted(async () => {
  console.log('[TaxiVoiceAssistant] Mounted')
...
const initSession = async () => {
  try {
    ...
    if (response.ok) {
      ...
      console.log('[TaxiVoiceAssistant] Session initialized:', data.session_id)
    } else {
      console.error('[TaxiVoiceAssistant] Failed to initialize session:', response.status)
    }
  } catch (error) {
    console.error('[TaxiVoiceAssistant] Session init error:', error)
...
const sendTextMessage = async (text) => {
  if (!text || !text.trim() || !chatStore.sessionId) {
    console.log('[TaxiVoiceAssistant] No text or session')
    return
  }

  textInput.value = ''
  console.log('[TaxiVoiceAssistant] Sending text:', text)
  ...
  try {
    ...
    if (response.ok) {
      const data = await response.json()
      console.log('[TaxiVoiceAssistant] Response:', data)
      ...
      if (data.visual_data) {
        console.log('[TaxiVoiceAssistant] Visual data:', data.visual_data)
        chatStore.setVisualData(data.visual_data)
      }
    } else {
      console.error('[TaxiVoiceAssistant] Error:', response.status)
      chatStore.addMessage('Sofia', 'Lo siento, hubo un error. Por favor intenta de nuevo.', 'assistant')
    }
  } catch (error) {
    console.error('[TaxiVoiceAssistant] Request error:', error)
...
const startRecording = async () => {
  try {
    ...
    console.error('[TaxiVoiceAssistant] Recording error:', error)
...
const sendVoiceMessage = async (audioBlob) => {
  if (!chatStore.sessionId) {
    console.log('[TaxiVoiceAssistant] No session for voice')
    return
  }

  console.log('[TaxiVoiceAssistant] Sending voice, size:', audioBlob.size)
  chatStore.setProcessing(true)
  ...
  try {
    ...
    if (response.ok) {
      const data = await response.json()
      console.log('[TaxiVoiceAssistant] Voice response:', data)
      ...
      console.error('[TaxiVoiceAssistant] Voice error:', response.status)
      chatStore.addMessage('Sofia', 'No pude procesar el audio.', 'assistant')
    }
  } catch (error) {
    console.error('[TaxiVoiceAssistant] Voice request error:', error)
...
const playAudio = async (audioUrl) => {
  try {
    ...
    audio.onerror = () => {
      console.error('[TaxiVoiceAssistant] Audio playback error')
      chatStore.setPlaying(false)
    }

    await audio.play()
  } catch (error) {
    console.error('[TaxiVoiceAssistant] Play audio error:', error)
    chatStore.setPlaying(false)
  }
}
</script>
